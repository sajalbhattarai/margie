...
if [[ ! -f "$DB_DMND" ]]; then
    local_fasta=""
    if [[ -f "/container/db/pepunit.lib" ]]; then
        local_fasta="/container/db/pepunit.lib"
    elif [[ -f "/container/db/pepunit.txt" ]]; then
        local_fasta="/container/db/pepunit.txt"
    fi

    if [[ -n "$local_fasta" ]]; then
        log "MEROPS DIAMOND DB missing; building from $local_fasta"
        diamond makedb --in "$local_fasta" -d "/container/db/merops" 2>&1 | tee -a "$LOG_FILE" || true
    fi
fi

if [[ ! -f "$DB_DMND" ]]; then
    log "ERROR: DIAMOND database not found: $DB_DMND"
    exit 1
fi
