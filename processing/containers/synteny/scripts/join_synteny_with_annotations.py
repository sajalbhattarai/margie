...
def discover_consolidated_files(root: Path) -> list[Path]:
    files_by_realpath: dict[Path, Path] = {}
    for fpath in root.rglob("*.tsv"):
        ...
        try:
            ...
            real = fpath
        files_by_realpath.setdefault(real, fpath)
    return sorted(files_by_realpath.values())


def infer_genome_name(fpath: Path, root: Path) -> str:
    name = fpath.name
    if name == "consolidated_annotations.tsv":
        return fpath.parent.name
    if name.startswith("consolidated_") and name.endswith(".tsv"):
        return name[len("consolidated_") : -len(".tsv")]
    if name.endswith("_consolidated.tsv"):
        return name[: -len("_consolidated.tsv")]
    if fpath.parent != root:
        return fpath.parent.name
    return fpath.stem
...
def load_consolidated_tables(consolidated_dir: str) -> pd.DataFrame:
    ...
    files = discover_consolidated_files(root)
    ...
    for fpath in files:
        try:
            ...
            if "feature_id" not in df.columns:
                continue
            df["genome_name"] = infer_genome_name(fpath, root)
            frames.append(df)
        except Exception:
            continue
